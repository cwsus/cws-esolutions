...
function addServiceIndicators
{
    ...
    LAST_SERIAL=$(grep "; serial" ${NAMED_ROOT}/${NAMED_ZONE_DIR}/${NAMED_MASTER_ROOT}/${ZONE_ROOT}/${FILENAME} | awk '{print $1}' | sed '/^$/d');
    ...
    then
        ...
        do
            ...
            if [ "${FAILOVER}" = "${_FALSE}" ]
            then
                sed "s/%${INDICATOR}%/${CHG_ARRAY[${A}]}/" ${APP_ROOT}/${TMP_DIRECTORY}/${FILENAME} > ${APP_ROOT}/${TMP_DIRECTORY}/${FILENAME}.tmp;
