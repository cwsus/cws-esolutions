            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        BACKUP_FILE=$(printf ${RNDC_KEY_FILE} | cut -d "/" -f 3).${CHANGE_NUM}.${TARFILE_DATE}.${REQUESTING_USER};

                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "BACKUP_FILE->${BACKUP_FILE}";

                        ## tar+gzip
                        tar cf ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar -C ${NAMED_ROOT}/etc/dnssec-keys $(printf ${RNDC_KEY_FILE} | cut -d "/" -f 3) \
                            $(printf ${RNDC_CONF_FILE} | cut -d "/" -f 3) > /dev/null 2>&1;
                        ...
                        then
                            ...
                            do
                                ...
                                MOD_FILE_CKSUM=$(cksum ${NAMED_ROOT}/etc/dnssec-keys/$(printf ${OPERATIONAL_FILE} | cut -d "/" -f 5) | awk '{print $1}' );
                                ...
                                then
                                    ...
                                    ## cksum mismatch, fail
                                    ${LOGGER} "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "CHECKSUM MISMATCH: ${OPERATIONAL_FILE} - ${OP_FILE_CKSUM} / $(printf ${NAMED_ROOT}/etc/dnssec-keys/$(printf ${OPERATIONAL_FILE} | cut -d "/" -f 4) - ${MOD_FILE_CKSUM})";
    ...
    then
        ...
        then
            ...
            then
                ...
                ## backup first
                TARFILE_DATE=$(date +"%m-%d-%Y");
                BACKUP_FILE=$(printf ${TRANSFER_KEY_FILE} | cut -d "/" -f 3).${CHANGE_NUM}.${TARFILE_DATE}.${REQUESTING_USER};

                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "BACKUP_FILE->${BACKUP_FILE}";

                ## tar+gzip
                tar cf ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar -C ${NAMED_ROOT}/etc/dnssec-keys $(printf ${TRANSFER_KEY_FILE} | cut -d "/" -f 3) > /dev/null 2>&1;
                gzip ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar > /dev/null 2>&1;
    ...
    then
        ...
        then
            ...
            then
                ...
                ## backup first
                TARFILE_DATE=$(date +"%m-%d-%Y");
                BACKUP_FILE=$(printf ${DHCPD_UPDATE_KEY} | cut -d "/" -f 3).${CHANGE_NUM}.${TARFILE_DATE}.${REQUESTING_USER};

                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "BACKUP_FILE->${BACKUP_FILE}";

                ## tar+gzip
                tar cf ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar -C ${NAMED_ROOT}/etc/dnssec-keys $(printf ${DHCPD_KEY_FILE} | cut -d "/" -f 3) > /dev/null 2>&1;
                gzip ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar > /dev/null 2>&1;
