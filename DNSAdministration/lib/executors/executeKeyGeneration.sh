...
function generateRNDCKeys
{
    ...
    then
        ...
        then
            ...
            [ -s ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3).REMOTE ] && rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3).REMOTE;
                ...
                then
                    ...
                    then
                        ...
                        then
                            ...
                            cp -p ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${RNDC_KEY_FILE};
                            cp -p ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${RNDC_CONF_FILE};

                            ## cksum verify
                            for OPERATIONAL_FILE in ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3) \
                                ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3)
                            do
    ...
    ## remove tmp files
    rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
    rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3);
        ...
        then
            ...
            then
                ...
                then
                    ...
                    ## now we can move the files in
                    cp -p ${PLUGIN_TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${TRANSFER_KEY_FILE};
                    ...
                    then
                        ## we're all set here. return the key back to the requestor.
                        ## remove tmp files
                        rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3);
                        rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3);
...
function generateDHCPDKeys
{
    ...
    ## remove the file if it exists
    [ -s ${PLUGIN_TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) ] && rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3);
    ...
    then
        ...
        if [ -s ${PLUGIN_TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) ]
        then
            ...
            then
                ...
                then
                    ...
                    ## now we can move the files in
                    cp -p ${PLUGIN_TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${DHCPD_KEY_FILE};

                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating checksums..";

                    OP_FILE_CKSUM=$(cksum ${PLUGIN_TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) | awk '{print $1}');
                    ...
                    then
                        ## we're all set here. return the key back to the requestor.
                        ## remove tmp files
                        rm -rf ${PLUGIN_TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3);
