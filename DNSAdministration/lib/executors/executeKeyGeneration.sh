...
function generateRNDCKeys
{
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command rndc-confgen -b ${RNDC_KEY_BITSIZE} -r ${RANDOM_GENERATOR} > ${NAMED_ROOT}/${TMP_DIRECTORY}..";
    ...
    then
        ...
        then
            ...
            [ -s ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3).REMOTE ] && rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3).REMOTE;
            ...
            printf "# keyfile ${RNDC_LOCAL_KEY} generated by ${IUSER_AUDIT} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
            printf "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
            printf "key \"${RNDC_LOCAL_KEY}\" {\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
            printf "    algorithm         hmac-md5;\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
            printf "    secret            \"${RETURN_KEY}\";\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
            ...
            then
                ...
                then
                    ...
                    ## once for the rndc conf file...
                    printf "# keyfile ${RNDC_REMOTE_KEY} generated by ${IUSER_AUDIT} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3);
                    printf "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3);
                    ...
                    then
                        ...
                        ## tar+gzip
                        tar cf ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar -C ${NAMED_ROOT}/etc/dnssec-keys $(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3) \
                        ...
                        then
                            ...
                            cp -p ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${RNDC_KEY_FILE};
                            cp -p ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${RNDC_CONF_FILE};

                            ## cksum verify
                            for OPERATIONAL_FILE in ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3) \
                                ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3)
                            do
                            ...
                            then
                                ...
                                ## remove tmp files
                                rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_KEY_FILE} | cut -d "/" -f 3);
                                rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${RNDC_CONF_FILE} | cut -d "/" -f 3);
...
function generateDNSSECKeys
{
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Setting backup file to ${NAMED_ROOT}/${BACKUP_DIRECTORY}/${FILENAME}.\`date +"%m-%d-%Y"\`.${CHANGE_NUM}.${IUSER_AUDIT}";
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Copying ${FILENAME} to ${NAMED_ROOT}/${TMP_DIRECTORY}..";

    ## make a copy of the zone for operation
    cp ${SITE_ROOT}/${FILENAME} ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${FILENAME};
    ...
    sed -e "1,${START_LINE_NUMBER}d" -e "s/${LAST_SERIAL}/${SERIAL_NUM}/" \
        ${NAMED_ROOT}/${NAMED_ZONE_DIR}/${NAMED_MASTER_DIR}/${GROUP_ID}${PROJECT_CODE}/${FILENAME} >> ${NAMED_ROOT}/${TMP_DIRECTORY}/${FILENAME};

    ## and now we should have two includes - lets make sure
    if [ $(grep -c INCLUDE ${NAMED_ROOT}/${TMP_DIRECTORY}/${FILENAME}) -ne 2 ]
    then
    ...
    mv ${NAMED_ROOT}/${TMP_DIRECTORY}/${FILENAME}.signed ${NAMED_ROOT}/${TMP_DIRECTORY}/${FILENAME};

    ## and then make sure it was created..
    if [ ! -s ${NAMED_ROOT}/${TMP_DIRECTORY}/${FILENAME} ]
    then
        ## file rename failed
    ...
    if [ $(grep -c dnssec_signzone ${NAMED_ROOT}/${TMP_DIRECTORY}/${FILENAME}) != 0 ]
    then
        ## shift failed
...
function generateTSIGKeys
{
    ...
    then
        ## this is a local rndc key. build the file
        printf "# keyfile ${TSIG_TRANSFER_KEY} generated by ${IUSER_AUDIT} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3);
        printf "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3);
        ...
        then
            ...
            then
                ...
                ## tar+gzip
                tar cf ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar -C ${NAMED_ROOT}/etc/dnssec-keys $(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3) > /dev/null 2>&1;
                gzip ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar > /dev/null 2>&1;
                ...
                then
                    ...
                    ## now we can move the files in
                    cp -p ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${TRANSFER_KEY_FILE};
                    ...
                    then
                        ## we're all set here. return the key back to the requestor.
                        ## remove tmp files
                        rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3);
                        rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${TRANSFER_KEY_FILE} | cut -d "/" -f 3);
...
function generateDHCPDKeys
{
    ...
    ## remove the file if it exists
    [ -s ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) ] && rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3);
    ...
    then
        ## this is a local rndc key. build the file
        printf "# keyfile ${DHCPD_UPDATE_KEY} generated by ${IUSER_AUDIT} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3);
        printf "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3);
        ...
        if [ -s ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) ]
        then
            ...
            then
                ...
                ## tar+gzip
                tar cf ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar -C ${NAMED_ROOT}/etc/dnssec-keys $(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) > /dev/null 2>&1;
                gzip ${PLUGIN_ROOT_DIR}/${BACKUP_DIRECTORY}/${BACKUP_FILE}.tar > /dev/null 2>&1;
                ...
                then
                    ...
                    ## now we can move the files in
                    cp -p ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${DHCPD_KEY_FILE};

                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && $(${LOGGER} "DEBUG" ${METHOD_NAME} ${CNAME} ${LINENO} "Validating checksums..");

                    OP_FILE_CKSUM=$(cksum ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3) | awk '{print $1}');
                    ...
                    then
                        ## we're all set here. return the key back to the requestor.
                        ## remove tmp files
                        rm -rf ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/$(echo ${DHCPD_KEY_FILE} | cut -d "/" -f 3);
...
function usage
{
    ...
    print " -r    -> Generate new RNDC keys. Requires a comma-delimited information set to execute the request.";
    print " -k    -> Generate DNSSEC keys. Requires a comma-delimited information set to execute the request.";
