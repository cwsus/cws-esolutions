        ...
        then
            ...
            then
                ...
                then
                    ...
                    ## once for the rndc conf file...
                    echo -n "# keyfile ${RNDC_REMOTE_KEY} generated by ${REQUESTING_USER} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${RNDC_CONF_FILE});
                    echo -n "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${RNDC_CONF_FILE});
                    echo -n "key \"${RNDC_REMOTE_KEY}\" {\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${RNDC_CONF_FILE});
                    echo -n "    algorithm         hmac-md5;\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${RNDC_CONF_FILE});
                    echo -n "    secret            \"${RETURN_KEY}\";\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${RNDC_CONF_FILE});
                    echo -n "};\n\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${RNDC_CONF_FILE});
...
function generateTSIGKeys
{
    ...
    then
        ## this is a local rndc key. build the file
        echo -n "# keyfile ${TSIG_TRANSFER_KEY} generated by ${REQUESTING_USER} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${TRANSFER_KEY_FILE});
        echo -n "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${TRANSFER_KEY_FILE});
        ...
        then
            ...
            ## echo -n back the key to the requestor for build against slaves
            if [ -s ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${TRANSFER_KEY_FILE}) ]
            then
                ## copy the keys in place
                ## backup first
...
function generateDHCPDKeys
{
    ...
    then
        ## this is a local rndc key. build the file
        echo -n "# keyfile ${DHCPD_UPDATE_KEY} generated by ${REQUESTING_USER} on ${KEY_GENERATION_DATE}\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${DHCPD_KEY_FILE});
        echo -n "# change request number ${CHANGE_NUM}\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${DHCPD_KEY_FILE});
        echo -n "key ${DHCPD_UPDATE_KEY} {\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${DHCPD_KEY_FILE});
        echo -n "    algorithm    hmac-md5;\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${DHCPD_KEY_FILE});
        echo -n "    secret       \"${RETURN_KEY}\";\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${DHCPD_KEY_FILE});
        echo -n "};\n\n" >> ${PLUGIN_WORK_DIRECTORY}/$(cut -d "/" -f 3 <<< ${DHCPD_KEY_FILE});
...
function usage
{
    ...
    echo -n "${CNAME} - Execute key generation requests";
    echo -n "Usage: ${CNAME} [ -r request information ] [ -k request information ] [-e] [-h|?]";
    echo -n " -r    -> Generate new RNDC keys. Requires a comma-delimited information set to execute the request.";
    echo -n " -k    -> Generate DNSSEC keys. Requires a comma-delimited information set to execute the request.";
...
do
    case "${OPTIONS}" in
        r)
            ...
            typeset -l KEYTYPE=$(echo -n "${OPTARG}" | cut -d "," -f 1);
            typeset -u CHANGE_NUM=$(echo -n "${OPTARG}" | cut -d "," -f 2);
