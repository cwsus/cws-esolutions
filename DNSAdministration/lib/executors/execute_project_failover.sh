...
#==============================================================================
#
#          FILE:  execute_project_failover.sh
#         USAGE:  ./execute_project_failover.sh [-v] [-b] [-f] [-t] [-p] [-h] [-?]
...
function failover_project
{
    ...
    local METHOD_NAME="${CNAME}#${0}";
            ...
            for FILENAME in $(ls -ltr ${APP_ROOT}/${TMP_DIRECTORY} | awk '{print $9}' | cut -d ":" -f 1-1 | grep -v "[PV]H" | uniq | sed '/ *#/d; /^ *$/d')
            do
...
function usage
{
    ...
    print "Usage: ${CNAME} [-p project code] [-b business unit] [-f named zone file] [-t target datacenter] [-p project code] [-h] [-?]";
    print " -p     The project code to fail over";
    print " -b     The business unit the project belongs to";
...
while getopts ":b:p:t:c:i:eh:" OPTIONS
do
    case "${OPTIONS}" in
        b)
        ...
        p)
            ...
            typeset -u PROJECT_CODE="${OPTARG}"; # This will be the PC to move
