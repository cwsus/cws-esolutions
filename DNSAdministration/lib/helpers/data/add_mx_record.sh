            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}) -ge 1 ] &&
                            [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}) -ge 1 ]
                        then
                            ...
                            if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ] &&
                                [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ]
                            then
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        ## make sure the record doesnt already exist in the zone
                        ## duplicates are just messy
                        if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}) -ge 1 ]
                        then
                            ## record already exists, return an error
                            ${LOGGER} "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "A record for ${RECORD_TYPE}, ${IP_ADDR} already exists. Cannot add duplicate.";
                            [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> exit";
                            ...
                            ## verify the record was written
                            if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}." ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}) -eq 1 ]
                            then
                                ## zone updated successfully
                                ${LOGGER} AUDIT "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Zone ${ZONEFILE_NAME} updated on $(date +"%m-%d-%Y") by ${IUSER_AUDIT} per change ${CHANGE_NUM}";
...
function add_subdomains
{
    ...
    then
        ...
        then
            ...
            if [ $(grep -c "${ALIAS}      IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}." ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ] &&
                [ $(grep -c "${ALIAS}      IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}." ${PLUGIN_ROOT_DIR}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ]
            then
