...
#==============================================================================
#
#          FILE:  add_mx_record.sh
#         USAGE:  ./add_mx_record.sh
...
function add_zone_addr
{
    ...
    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "checking for ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}..";
    ...
    then
        ...
        then
            ...
            then
                ...
                if [ $(echo ${IP_ADDR} | tr -dc ',' | wc -c) -ne 1 ]
                then
                    ${LOGGER} ERROR "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Record information provided was either blank or does not contain enough information. Cannot continue.";
                    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> exit";
                    RETURN_CODE=30;
                else
                    PRIORITY=$(echo ${IP_ADDR} | cut -d "," -f 1);
                    IPADDR=$(echo ${IP_ADDR} | cut -d "," -f 2);

                    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "PRIORITY->${PRIORITY}";
                    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "IPADDR->${IPADDR}";

                    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating information..";

                    ## check to make sure we have the information, if not, throw an error
                    if [ -z "${PRIORITY}" ] || [ -z "${IPADDR}" ] || [ "$(echo ${PRIORITY} | grep ^[0-9])" != "" ]
                    then
                        ${LOGGER} ERROR "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Record information provided was either blank or does not contain enough information. Cannot continue.";
                        [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> exit";
                        RETURN_CODE=30;
                        ...
                        if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}) -ge 1 ] &&
                            [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}) -ge 1 ]
                        then
                            ...
                            printf "                IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}\n" >> ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}
                            printf "                IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}\n" >> ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}

                            ## verify the record was written
                            if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ] &&
                                [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ]
                            then
                                ...
                                ${LOGGER} ERROR "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Record addition to ${ZONE_NAME} FAILED. Please try again.";
                                [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> exit";
                                RETURN_CODE=42;
                            fi
                        fi
        ...
        if [ -s ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME} ]
        then
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        ## make sure the record doesnt already exist in the zone
                        ## duplicates are just messy
                        if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}" ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}) -ge 1 ]
                        then
                            ## record already exists, return an error
                            ${LOGGER} ERROR "${METHOD_NAME}" "${CNAME}" "${LINENO}" "A record for ${RECORD_TYPE}, ${IP_ADDR} already exists. Cannot add duplicate.";
                            [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> exit";
                            ...
                            printf "                IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}.\n" >> ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}

                            ## verify the record was written
                            if [ $(grep -c "${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}." ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}) -eq 1 ]
                            then
                                ## zone updated successfully
                                ${LOGGER} AUDIT "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Zone ${ZONEFILE_NAME} updated on $(date +"%m-%d-%Y") by ${IUSER_AUDIT} per change ${CHANGE_NUM}";
...
function add_subdomains
{
    ...
    then
        ...
        PRIORITY=$(echo ${IP_ADDR} | cut -d "," -f 2);
        IPADDR=$(echo ${IP_ADDR} | cut -d "," -f 3);
        ...
        if [ -z "${ALIAS}" ] || [ -z "${PRIORITY}" ] || [ -z "${IPADDR}" ] || [ "$(echo ${PRIORITY} | grep ^[0-9])" != "" ]
        then
            ## something was blank, or the priority wasnt a number
            ${LOGGER} ERROR "${METHOD_NAME}" "${CNAME}" "${LINENO}" "A required entry was either blank or the priority was non-numeric. Cannot continue.";
            RETURN_CODE=30;
        else
            printf "${ALIAS}      IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}.\n" >> ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME};
            printf "${ALIAS}      IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}.\n" >> ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME};

            ## validate the record was written
            if [ $(grep -c "${ALIAS}      IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}." ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${PRIMARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ] &&
                [ $(grep -c "${ALIAS}      IN      ${RECORD_TYPE}      ${PRIORITY}      ${IPADDR}." ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${SECONDARY_DC}/${DC_ZONEFILE_NAME}) -eq 1 ]
            then
                ## record was successfully written
                [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Printed MX record to ${DC_ZONEFILE_NAME}";
