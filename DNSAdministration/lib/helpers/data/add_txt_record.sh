...
#==============================================================================
#
#          FILE:  add_txt_record.sh
#         USAGE:  ./add_txt_record.sh
    ...
    then
        ...
        then
            ...
            then
                ...
                ## txt records. we treat them alone and perform almost
                ## no validation due to the possible complexity
                ## txt records are usually rather complex. for instance:
                ## "v=spf1 a mx ip4:207.79.183.238 -all"
                ## we cant really validate this due to the complexity
                ## of the record type - so we just print it out
        ...
        then
            ...
            then
                ...
                then
                    ...
                    ## validate the record was written
                    if [ $(grep -c "${ALIAS}      IN      ${RECORD_TYPE}      \"${IPADDR}\"" ${APP_ROOT}/${TMP_DIRECTORY}/${GROUP_ID}${BUSINESS_UNIT}/${DATACENTER}/${DC_ZONEFILE_NAME}) -eq 1 ]
                    then
                        ## record was successfully written
                        [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} DEBUG "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Printed ${RECORD_TYPE} record to ${DC_ZONEFILE_NAME}";
