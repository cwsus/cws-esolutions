...
function add_root_ui_helper
{
    ...
    do
        ...
        print "\t\t\t$(grep system.application.title ${PLUGIN_CONFIG} | grep -v "#" | cut -d "=" -f 2)\n";
        print "\t$(grep add.enter.ipaddr.primary ${PLUGIN_CONFIG} | grep -v "#" | cut -d "=" -f 2 | sed -e "s/%PRIMARY_DATACENTER%/${PRIMARY_DC}/")";
    ...
    local METHOD_NAME="${CNAME}#${0}";
                    ...
                    then
                        ...
                        do
                            ...
                            print "\t\t\t$(grep system.application.title ${PLUGIN_CONFIG} | grep -v "#" | cut -d "=" -f 2)\n";
                            print "\t$(grep add.enter.ipaddr.secondary ${PLUGIN_CONFIG} | grep -v "#" | cut -d "=" -f 2 | sed -e "s/%SECONDARY_DATACENTER%/${SECONDARY_DC}/")";
                            print "\t$(grep system.option.cancel "${SYSTEM_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)\n";
