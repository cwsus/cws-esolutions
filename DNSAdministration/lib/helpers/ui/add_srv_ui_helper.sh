...
function add_subdomain_helper
{
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> enter";
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Requesting subdomain SRV record information..";
                                                                                                                        ...
                                                                                                                        case ${SERVICE_TARGET} in
                                                                                                                            ...
                                                                                                                            *)
                                                                                                                                ...
                                                                                                                                then
                                                                                                                                    ...
                                                                                                                                    ## our service target is valid. we can write out the record.
                                                                                                                                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing add_${RECORD_TYPE}_record.sh -p kvn -z kevin.com -i h33355 -c C991882 -t srv -a ${SERVICE_TYPE},${SERVICE_PROTO},${SERVICE_NAME},${SERVICE_TTL},${SERVICE_PRIORITY},${SERVICE_WEIGHT},${SERVICE_PORT},${SERVICE_TARGET} -s";
