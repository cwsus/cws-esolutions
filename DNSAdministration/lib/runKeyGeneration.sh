    ...
    then
        ...
        then
            ...
            do
                ...
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command ${PLUGIN_LIB_DIRECTORY}/tcl/runSCPConnection.exp remote-copy ${AVAILABLE_MASTER_SERVERS} ${NAMED_ROOT}/${RNDC_KEY_FILE} ${PLUGIN_WORK_DIRECTORY}/$(printf ${RNDC_KEY_FILE} | cut -d "/" -f 3).MASTER";

                ${APP_ROOT}/${LIB_DIRECTORY}/tcl/runSCPConnection.exp remote-copy ${AVAILABLE_MASTER_SERVERS} ${NAMED_ROOT}/${RNDC_KEY_FILE} ${PLUGIN_WORK_DIRECTORY}/$(printf ${RNDC_KEY_FILE} | cut -d "/" -f 3).MASTER ${SSH_USER_NAME} ${SSH_USER_AUTH};

                ## make sure we got it
                if [ -s ${PLUGIN_WORK_DIRECTORY}/$(printf ${RNDC_KEY_FILE} | cut -d "/" -f 3).MASTER ]
                then
                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command ${APP_ROOT}/${LIB_DIRECTORY}/tcl/runSCPConnection.exp remote-copy ${AVAILABLE_MASTER_SERVERS} ${NAMED_ROOT}/${RNDC_KEY_FILE} ${PLUGIN_WORK_DIRECTORY}/$(printf ${RNDC_CONF_FILE} | cut -d "/" -f 3).MASTER";

                    ${APP_ROOT}/${LIB_DIRECTORY}/tcl/runSCPConnection.exp remote-copy ${AVAILABLE_MASTER_SERVERS} ${NAMED_ROOT}/${RNDC_CONF_FILE} ${PLUGIN_WORK_DIRECTORY}/$(printf ${RNDC_CONF_FILE} | cut -d "/" -f 3).MASTER ${SSH_USER_NAME} ${SSH_USER_AUTH};

                    ## make sure we got it
                    if [ -s ${PLUGIN_WORK_DIRECTORY}/$(printf ${RNDC_CONF_FILE} | cut -d "/" -f 3).MASTER ]
                    then
...
function generateTSIGKeys
{
    ...
    then
        ...
        do
            ...
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "OP_FILE_CKSUM -> ${OP_FILE_CKSUM}";
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command ${APP_ROOT}/${LIB_DIRECTORY}/tcl/runSCPConnection.exp local-copy ${DNS_SLAVES[${A}]} ${PLUGIN_WORK_DIRECTORY}/$(printf ${TRANSFER_KEY_FILE} | cut -d \"/\" -f 3) ${NAMED_ROOT}/${TRANSFER_KEY_FILE}";
            ...
            ## validate the input
            ${APP_ROOT}/${LIB_DIRECTORY}/tcl/runSCPConnection.exp local-copy ${DNS_SLAVES[${A}]} ${PLUGIN_WORK_DIRECTORY}/$(printf ${TRANSFER_KEY_FILE} | cut -d "/" -f 3) ${NAMED_ROOT}/${TRANSFER_KEY_FILE} ${SSH_USER_NAME} ${SSH_USER_AUTH};
            typeset -i RET_CODE=${?};
