...
CREATE TABLE CWSSEC.AUDIT (
    ...
    CONSTRAINT FK_LGN_GUID
        ...
        REFERENCES CWSSEC.USERS (CN)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
    FULLTEXT KEY FT_AUDIT (USERNAME, CN, SOURCE_ADDRESS, SOURCE_HOSTNAME, ACTION)
...
BEGIN
    SELECT
        ...
        SOURCE_ADDRESS,
        SOURCE_HOSTNAME,
    MATCH (USERNAME, CN, SOURCE_ADDRESS, SOURCE_HOSTNAME, ACTION)
    AGAINST (+attributeName WITH QUERY EXPANSION)
    FROM CWSSEC.AUDIT
    WHERE MATCH (USERNAME, CN, SOURCE_ADDRESS, SOURCE_HOSTNAME, ACTION)
...
BEGIN
    INSERT INTO AUDIT (SESSION_ID, USERNAME, CN, APPLICATION_ID, APPLICATION_NAME, REQUEST_TIMESTAMP, ACTION, SOURCE_ADDRESS, SOURCE_HOSTNAME)
    VALUES (usersessid, username, userguid, applid, applname, CURRENT_TIMESTAMP(), useraction, srcaddr, srchost);
END $$
/*!50003 SET SESSION SQL_MODE=@TEMP_SQL_MODE */  $$
COMMIT$$
