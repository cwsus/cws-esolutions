...
CREATE TABLE `esolutionssvc`.`service_projects` (
    ...
    `PROD_EMAIL` VARCHAR(128) NOT NULL,
    `INCIDENT_QUEUE` VARCHAR(128) NOT NULL,
    `CHANGE_QUEUE` VARCHAR(128) NOT NULL,
    PRIMARY KEY (`PROJECT_GUID`),
    FULLTEXT KEY `IDX_PROJECTS` (`PROJECT_CODE`, `PROJECT_STATUS`, `PRIMARY_OWNER`, `SECONDARY_OWNER`, `DEV_EMAIL`, `PROD_EMAIL`)
...
BEGIN
    ...
    MATCH (`PROJECT_CODE`, `PROJECT_STATUS`, `PRIMARY_OWNER`, `SECONDARY_OWNER`, `DEV_EMAIL`, `PROD_EMAIL`)
    AGAINST (+attributeName WITH QUERY EXPANSION)
    FROM `esolutionssvc`.`service_projects`
    WHERE MATCH (`PROJECT_CODE`, `PROJECT_STATUS`, `PRIMARY_OWNER`, `SECONDARY_OWNER`, `DEV_EMAIL`, `PROD_EMAIL`)
...
BEGIN
    ...
    (PROJECT_GUID, PROJECT_CODE, PROJECT_STATUS, PRIMARY_OWNER, SECONDARY_OWNER, DEV_EMAIL, PROD_EMAIL, INCIDENT_QUEUE, CHANGE_QUEUE)
    VALUES
    (projectGuid, projectCode, status, primaryOwner, secondaryOwner, devEmail, prodEmail, incidentQueue, changeQueue);
