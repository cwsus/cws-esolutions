...
function addApexRecordEntry
{
    ...
    then
        ...
        if [ -z ${RET_CODE} ] || [ ${RET_CODE} -ne 0 ]
        then
            "${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WARN: The provided address is not an IP. If this is intentional, this warning can be ignored.";
        fi
    fi
    ...
    then
        ...
        [ -z ${RET_CODE} ] || [ ${RET_CODE} -ne 0 ] && "${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WARNING: The provided record data could not be located.";
    fi
        ...
        [Aa]|[Nn][Ss])
            ...
            do
                ...
                then
                    ...
                    "${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Record for ${RECORD_TYPE}, ${RECORD_DATA} already exists. Cannot add duplicate.";
