...
[ ! -z "${ENABLE_TRACE}" ] && [ "${ENABLE_TRACE}" = "${_TRUE}" ] && set -v;
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> enter";
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Provided arguments: ${*}";
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Checking synchronization status..";

    ## start processing records
    for CONFIG_ENTRY in $(find "${NAMED_ROOT}/${NAMED_CONF_DIR}" -type f -name *.${NAMED_ZONE_CONF_NAME} -print)
    do
        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "CONFIG_ENTRY -> ${CONFIG_ENTRY}";
        ...
        do
            ...
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "MASTER_SOA_SERIAL -> ${MASTER_SOA_SERIAL}";
            ...
            then
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "MASTER_SOA_SERIAL -> is not a serial number. Obtaining..";
                ...
                then
                    writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "MASTER_SOA_SERIAL for ${ZONE_ENTRY} did not resolve to a number. Recursion level reached, failing out.";
                fi
            else
                ...
                then
                    ...
                    do
                        ...
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "SLAVE_SOA_SERIAL -> ${SLAVE_SOA_SERIAL}";

                        if [ "$(isNaN ${MASTER_SOA_SERIAL})" = "${_FALSE}" ]
                        then
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "SLAVE_SOA_SERIAL -> is not a serial number. Obtaining..";
                            ...
                            then
                                writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "SLAVE_SOA_SERIAL for ${ZONE_ENTRY} did not resolve to a number. Recursion level reached, failing out.";
                            else
                                if [ ${MASTER_SOA_SERIAL} != ${SLAVE_SOA_SERIAL} ]
                                then
                                    writeLogEntry "MONITOR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Serial number for zone ${ZONE_ENTRY} on ${NAMED_MASTER} does NOT match slave ${SLAVE_SERVER}. Master serial: ${MASTER_SOA_SERIAL}, Slave serial: ${SLAVE_SOA_SERIAL}";
                                fi
                            fi
                        else
                            if [ ${MASTER_SOA_SERIAL} != ${SLAVE_SOA_SERIAL} ]
