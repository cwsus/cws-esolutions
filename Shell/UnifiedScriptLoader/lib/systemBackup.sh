...
function performBackup
{
    ...
    do
        ...
        then
            "${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Failed to zip backup file.";
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        if [ -z "${BACKUP_LOADED}" ]
                        then
                            "${LOGGER}" "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Unable to load backup configuration. Cannot continue.";
