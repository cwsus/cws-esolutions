...
SCRIPT_ABSOLUTE_PATH="$(cd "${0%/*}" 2>/dev/null; /usr/bin/env printf "${PWD}"/"${0##*/}")";
...
function main
{
    ...
    then
        ...
        trap "printf '$(grep -w system.trap.signals "${SYSTEM_MESSAGES}" | grep -v "#" | cut -d "=" -f 2 | sed -e "s/%SIGNAL%/Ctrl-C/")'; sleep ${MESSAGE_DELAY}; reset; clear; continue " 1 2 3
        ...
        do
            ...
            printf "\t$(grep -w cert.mgmt.create.new.cert "${PLUGIN_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)";
            printf "\t$(grep -w cert.mgmt.renew.cert "${PLUGIN_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)";
            printf "\t$(grep -w cert.mgmt.exclude.cert "${PLUGIN_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)";
            printf "\t$(grep -w cert.mgmt.run.adhoc.report "${PLUGIN_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)";
            printf "\t$(grep -w system.option.cancel "${SYSTEM_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)\n";
            ...
            case ${REQUEST_OPTION} in
                [Xx]|[Qq]|[Cc])
                    ...
                    printf "$(grep -w system.request.canceled "${SYSTEM_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)\n";
...
typeset RETURN_CODE=0;
        ...
        printf "$(grep -w request.not.authorized "${ERROR_MESSAGES}" | grep -v "#" | cut -d "=" -f 2)\n";
