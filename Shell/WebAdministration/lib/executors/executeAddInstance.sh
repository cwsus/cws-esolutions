...
function installiPlanetInstance
{
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> enter";
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "SERVER_ID -> ${SERVER_ID}";
    ...
    ## untar it
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating tarball..";

    if [ -s "${APP_ROOT}"/${BUILD_TMP_DIR}/${IUSER_AUDIT}-${SERVER_ID}.tar.gz ]
    then
        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Tarball validated. Exploding web instance into ${IPLANET_ROOT} ..";

        gzip -dc "${APP_ROOT}"/${BUILD_TMP_DIR}/${IUSER_AUDIT}-${SERVER_ID}.tar.gz | (cd ${IPLANET_ROOT}; tar xf -);

        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Explosion complete. Validating ..";
        ...
        then
            ...
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Adding ${BIND_ADDR} to ${IPLANET_SERVER_CONFIG}..";
            ...
            then
                ...
                then
                    ## confirmed. relocate
                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Modification verified. Renaming ..";
                    ...
                    then
                        ...
                        then
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validation complete. Confirming directory access and creating as necessary..";
                            ...
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WEB_LOG_ROOT -> ${WEB_LOG_ROOT}";
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WEB_DOC_ROOT -> ${WEB_DOC_ROOT}";
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WEB_TMP_DIR -> ${WEB_TMP_DIR}";

                            for DIRECTORY in ${WEB_LOG_ROOT} ${WEB_DOC_ROOT} ${WEB_TMP_DIR}
                            do
                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating directory ${DIRECTORY} ..";

                                if [ ! -d ${DIRECTORY} ]
                                then
                                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Creating ${DIRECTORY} ..";

                                    mkdir -p ${DIRECTORY};

                                    if [ ! -d ${DIRECTORY} ]
                                    then
                                        writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Failed to make directory ${DIRECTORY}.";
                            ...
                            then
                                ...
                                do
                                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating file ${SERVER_ID}-${IUSER_AUDIT}-${SUFFIX}..";

                                    if [ -s ${IPLANET_ROOT}/${IPLANET_CERT_DIR}/${SERVER_ID}-${IUSER_AUDIT}-${SUFFIX} ]
                                    then
                                        ## rename it
                                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Renaming certificate database ${SERVER_ID}-${IUSER_AUDIT}-${SUFFIX}..";
                                        ...
                                        then
                                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Rename validated.";

                                            (( FILE_COUNT += 1 ));
                                        else
                                            ## rename failed
                                            writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Failed to rename certificate database.";

                                            (( ERROR_COUNT += 1 ));
                                        fi
                                    else
                                        ## unable to locate certificate database
                                        writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Unable to locate certificate database ${SERVER_ID}-${IUSER_AUDIT}-${SUFFIX} in ${IPLANET_ROOT}/${IPLANET_CERT_DIR}.";
                                ...
                                then
                                    ...
                                    then
                                        ...
                                        do
                                            ...
                                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "ACL_FILE -> ${ACL_FILE}";
                                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating file ${ACL_FILE} ..";

                                            if [ -s ${IPLANET_ROOT}/${IPLANET_ACL_DIR}/${ACL_FILE} ]
                                            then
                                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${ACL_FILE} validated.";

                                                (( FILE_COUNT += 1 ));
                                            else
                                                ## unable to locate certificate database
                                                writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Unable to locate ${ACL_FILE} in ${IPLANET_ROOT}/${IPLANET_ACL_DIR}.";
                                        ...
                                        then
                                            ...
                                            then
                                                ...
                                                writeLogEntry "AUDIT" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Server installation successful: Server ID: ${SERVER_ID}, Requestor: ${IUSER_AUDIT}.";

                                                ## clean up temporary files
                                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Removing temporary installation files..";

                                                rm -rf "${APP_ROOT}"/${BUILD_TMP_DIR}/${IUSER_AUDIT}-${SERVER_ID}.tar "${APP_ROOT}"/${BUILD_TMP_DIR}/${IUSER_AUDIT}-${SERVER_ID}.tar.gz > /dev/null 2>&1;

                                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Cleanup complete. Setting permissions on scripts ..";
                                                ...
                                                do
                                                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "SCRIPT -> ${SCRIPT}";

                                                    chmod 754 ${IPLANET_ROOT}/${SERVER_ID}/${SCRIPT} > /dev/null 2>&1;
                                                done

                                                ## start the server
                                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Permission changes complete. Starting server..";
                                                ...
                                                if [ ${RET_CODE} -eq 0 ]
                                                then
                                                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Server started successfully.";

                                                    RETURN_CODE=0;
                                                else
                                                    writeLogEntry writeLogEntry "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WARNING: Server failed to properly start up. Please investigate server logs.";

                                                    RETURN_CODE=40;
                                                fi
                                            else
                                                ## error occurred copying acl files
                                                writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Failed to install ACL files. Please install manually.";
                                        ...
                                        writeLogEntry "AUDIT" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "One or more errors occurred while copying certificate databases. Cannot continue.";

                                        RETURN_CODE=8;
                                    fi
                                else
                                    ## one or more cert databases aren't here
                                    writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate database copy failed. Cannot continue.";

                                    RETURN_CODE=8;
                                fi
                            else
                                ## one or more web directories failed to create
                                writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "One or more web directories failed to create. Cannot continue.";

                                RETURN_CODE=50;
                            fi
                        else
                            ## failed to validate bind address
                            writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Unable to insert binding interface address. Cannot continue.";
            ...
            writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Failed to install new web instance. Cannot continue.";

            RETURN_CODE=32;
        fi
    else
        ## tarfile doesnt exist
        writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Unable to locate web instance package. Cannot continue.";
...
do
    case "${OPTIONS}" in
        s)
            ...
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Setting SERVER_ID..";
        ...
        e)
            ...
            then
                if [ -z "${SERVER_ID}" ]
                then
                    writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "No server id was provided. Unable to continue processing.";
                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${METHOD_NAME} -> exit";
                ...
                then
                    ...
                    then
                        ...
                        then
                            ...
                            writeLogEntry "ERROR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "No valid platform type was found. Cannot continue.";
