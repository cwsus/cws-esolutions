        ...
        then
            ...
            then
                ...
                do
                    ...
                    then
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Checking for ${OPTIONS_MONITOR_STRING} on ${WEBSERVER}..";
                        ...
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "IS_METHOD_ENABLED -> ${IS_METHOD_ENABLED}";

                        if [ ! -z "${IS_METHOD_ENABLED}" ]
                        then
                            ## ok, good - its all blocked out. make sure trace isnt enabled via svc
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Checking for ${SVCTRACE_MONITOR_STRING} on ${WEBSERVER}..";
                            ...
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && writeLogEntry "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "IS_TRACE_ENABLED -> ${IS_TRACE_ENABLED}";

                            if [ ! -z "${IS_TRACE_ENABLED}" ]
                            then
                                ## one or more obj files have trace enabled. although its blocked above,
                                ## should prolly clean it up anyway
                                writeLogEntry "MONITOR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Service TRACE is enabled on ${WEBSERVER}";
                            fi
                        else
                            ## one or more obj files arent blocking the "bad" methods
                            writeLogEntry "MONITOR" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Blocking of the following options: TRACE|TRACK|OPTIONS|PUT|DELETE - not found on ${WEBSERVER}";
                        fi
                    fi
                done
