...
function renewiPlanetCert
{
    ...
    then
        ...
        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Cleanup complete. Validating..";

        if [ -s "${APP_ROOT}"/${CERTSTORE}/${CERTIFICATE_NICKNAME}.cer ]
        then
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validation complete. Applying certificate..";
            ...
            then
                ...
                $("${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}"ING "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate removal FAILED.");
            ...
            then
                ...
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "RENEWED_CERTIFICATE_NICKNAME -> ${RENEWED_CERTIFICATE_NICKNAME}";
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "POST_CERT_EXPIRY -> ${POST_CERT_EXPIRY}";
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "NOTIFY_RENEWED_CERT_EXPIRY -> ${NOTIFY_RENEWED_CERT_EXPIRY}";
                ...
                then
                    ...
                    then
                        ...
                        then
                            ...
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate successfully applied to certificate databases. Generating PKCS12 file..";
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && $("${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command pk12util -o "${APP_ROOT}"/${PKCS12STORE}/${CERTIFICATE_NICKNAME}.pkcs" \
                            ...
                            then
                                ...
                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate export successful. Validating..";

                                if [ -s "${APP_ROOT}"/${PKCS12STORE}/${CERTIFICATE_NICKNAME}.pkcs ]
                                then
                                    ## it did. find out if this is a tealeaf site
                                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate export successfully validated. Checking TeaLeaf data..";

                                    if [ $(grep -c ${SITE_DOMAIN_NAME} "${APP_ROOT}"/${PEM_SITES_LIST}) != 0 ]
                                    then
                                        ## ok, we know we need to generate a pem. lets do it
                                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${SITE_DOMAIN_NAME} has a TeaLeaf requirement. Generating PEM file..";
                                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && $("${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command openssl pkcs12 -nodes -nocerts -in " \
...
typeset RETURN_CODE=0;
                                                ...
                                                if [ ${MAILER_CODE} -ne 0 ]
                                                then
                                                    ## error occurred sending the pem, "${LOGGER}" "AUDIT" "${METHOD_NAME}"
                                                    "${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "PEM file send FAILED. Please process manually.";
...
function renewIHSCert
{
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "CERTIFICATE_NICKNAME -> ${CERTIFICATE_NICKNAME}";
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "CERTIFICATE_EXPIRY -> ${CERTIFICATE_EXPIRY}";

    if [ -s "${APP_ROOT}"/${CERTSTORE}/${CERTIFICATE_DATABASE}.cer ]
    then
        ## got it. clean it up if necessary
        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate validated. Removing extra spaces and linebreaks..";
        ...
        then
            ...
            ## xlnt. apply it.
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate validated. Continuing..";
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command keyman -cert -receive -db "${APP_ROOT}"/${CERTDB_STORE}/${CERTIFICATE_DATABASE}${IHS_DB_CRT_SUFFIX} -file "${APP_ROOT}"/${CERT_STORE}/${CERTIFICATE_NICKNAME}.cer -pw $(cat "${APP_ROOT}"/${IHS_CERT_DB_PASSFILE}) -type ${IHS_KEY_DB_TYPE} -format ascii";
            ...
            then
                ...
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "RENEWED_CERT_NICKNAME -> ${RENEWED_CERT_NICKNAME}";
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "RENEWED_CERT_EXPIRY -> ${RENEWED_CERT_EXPIRY}";
                ...
                then
                    ...
                    then
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate successfully applied to certificate databases. Generating PKCS12 file..";
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command keyman -cert -export -db "${APP_ROOT}"/${CERTDB_STORE}/${CERTIFICATE_NICKNAME}${IHS_DB_CRT_SUFFIX} -label ${CERTIFICATE_NICKNAME} -target "${APP_ROOT}"/${PKCS12STORE}/${CERTIFICATE_NICKNAME}.pkcs -pw $(cat "${APP_ROOT}"/${IHS_CERT_DB_PASSFILE}) -target_pw $(cat "${APP_ROOT}"/${IHS_CERT_DB_PASSFILE}) -target_type pkcs12 -encryption strong";
                        ...
                        then
                            ...
                            then
                                ## ok, we know we need to generate a pem. lets do it
                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "${SITE_DOMAIN_NAME} has a TeaLeaf requirement. Generating PEM file..";
                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command openssl pkcs12 -nodes -nocerts -in "${APP_ROOT}"/${PKCS12STORE}/${CERTIFICATE_NICKNAME}.pkcs -out "${APP_ROOT}"/${PEMSTORE}/${CERTIFICATE_NICKNAME}.pem -password file:"${APP_ROOT}"/${IHS_CERT_DB_PASSFILE}";
...
function applyiPlanetCert
{
    ...
    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "WEB_INSTANCE_NAME -> ${WEB_INSTANCE_NAME}";
        ...
        then
            ...
            then
                ...
                then
                    ...
                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "BACKUP_TARFILE -> ${BACKUP_TARFILE}";
                    ...
                    then
                        ## backup complete
                        ## remove temp files
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Backup validated. Removing working files..";
                        ...
                        "${LOGGER}" "${LOGGER}" "AUDIT" "${METHOD_NAME}" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "An error occurred generating the backup file. Please process manually.";
...
function generateAndApplySelfSignedCert
{
    ...
    then
        ...
        then
            if [ ${RET_CODE} -eq 0 ]
            then
                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Validating certificate creation..";

                if [ -s "${APP_ROOT}"/${CERTSTORE}/SS-${CERT_NICKNAME}.cer ]
                then
                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Certificate created. Re-distributing text..";
                    ...
                    [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Distribution complete. Validating..";

                    if [ -s "${APP_ROOT}"/${CERTSTORE}/SS-${CERT_NICKNAME}.cer.tmp ]
                    then
                        [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "File created. Validating text..";
                        ...
                        then
                            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Text validated. Moving file..";
                            ...
                            if [ -s "${APP_ROOT}"/${CERTSTORE}/SS-${CERT_NICKNAME}.cer ]
                            then
                                [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "File created, validating..";
...
do
    case "${OPTIONS}" in
        ...
        a)
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "OPTARG -> ${OPTARG}";
            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Setting CLUSTER_APPLICATION to ${_TRUE}..";

            ## Capture the change control
            CLUSTER_APPLICATION=${_TRUE};

            [ ! -z "${ENABLE_DEBUG}" ] && [ "${ENABLE_DEBUG}" = "${_TRUE}" ] && "${LOGGER}" "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "CLUSTER_APPLICATION -> ${CLUSTER_APPLICATION}";
            ;;
        e)
