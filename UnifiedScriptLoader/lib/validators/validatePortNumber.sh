...
function checkIfPortFree
{
    ...
    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "PORT_CHECK_RESULTS -> ${PORT_CHECK_RESULTS}";

    [ ! -z "${PORT_CHECK_RESULTS}" ] && VALIDATE_CODE=1 || VALIDATE_CODE=0;
