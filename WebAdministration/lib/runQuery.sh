...
function returnARecord
{
    ...
    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing DiG query for ${SITE_URL}..";
    ...
    then
        ...
        then
            ...
            [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && $(${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command wget -q -O - \"\${@}\" " \
            ...
            do
                ...
                then
                    ...
                    [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Executing command ${APP_ROOT}/lib/tcl/runSSHConnection.exp ${PROXY} \"wget -q -O - \"\${@}\" $(echo ${DNS_SERVICE_URL} | sed -e "s/{SITE_HOSTNAME}/${SITE_URL}/") > ${APP_ROOT}/${DIG_DATA_FILE}";
    ...
    then
        ...
        then
            ...
            then
                ...
                [[ ! -z "${VERBOSE}" && "${VERBOSE}" = "${_TRUE}" ]] && ${LOGGER} "DEBUG" "${METHOD_NAME}" "${CNAME}" "${LINENO}" "Alternate datcenter located. ALTERNATE_DATACENTER -> ${ALTERNATE_DATACENTER}";
