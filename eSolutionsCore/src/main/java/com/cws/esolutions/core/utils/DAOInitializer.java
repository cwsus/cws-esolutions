...
public final class DAOInitializer
{
    ...
    {
        String methodName = DAOInitializer.CNAME + "#configureAndCreateConnection(final String properties, final boolean isContainer) throws CoreServiceException";
            ...
            switch (repoType)
            {
                ...
                case SQL:
                    ...
                    if (isContainer)
                    {
                        ...
                        return envContext.lookup(DAOInitializer.REPOSITORY_HOST);
                    }
                    else
                    {
                ...
                case NONE:
                	throw new CoreServiceException("No resource type was provided. Cannot continue.");
                default:
        ...
        {
            throw new CoreServiceException(fnfx.getMessage(), fnfx);
        }
    ...
    {
        String methodName = DAOInitializer.CNAME + "#closeOpenConnection(final Object dataObject, final boolean isContainer)";

        if (DEBUG)
        {
            DEBUGGER.debug(methodName);
            DEBUGGER.debug("Object: {}", dataObject);
            DEBUGGER.debug("isContainer: {}", isContainer);
        }

        try
        {
            if (dataObject instanceof LDAPConnectionPool)
            {
            	LDAPConnectionPool ldapPool = (LDAPConnectionPool) dataObject;
            ...
            else if (dataObject instanceof BasicDataSource)
            {
            	BasicDataSource dataSource = (BasicDataSource) dataObject;
