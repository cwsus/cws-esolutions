...
public class KnowledgeBaseController
{
    ...
    public final void setMessageAddArticleSuccess(final String value)
    {
        final String methodName = KnowledgeBaseController.CNAME + "#setMessageAddArticleSuccess(final String value)";
        ...
        this.messageAddArticleSuccess = value;
    }
    ...
    public final ModelAndView showSearchPage(@PathVariable("terms") final String terms, @PathVariable("page") final int page)
    {
        ...
        if (this.appConfig.getServices().get(this.serviceName))
        {
            try
            {
                ...
                SearchResponse response = searchProcessor.doArticleSearch(request);
            ...
            try
            {
                ...
                if (response.getRequestStatus() == CoreServicesStatus.SUCCESS)
                {
                    ...
                    else
                    {
                        mView.addObject("command", new SearchRequest());
                        mView.addObject("isHelpSearch", true);
                        mView.addObject(Constants.MESSAGE_RESPONSE, this.appConfig.getMessageNoSearchResults());
                        mView.setViewName(this.defaultPage);
                    }
                }
                else if (response.getRequestStatus() == CoreServicesStatus.UNAUTHORIZED)
        ...
        if (this.appConfig.getServices().get(this.serviceName))
        {
            ...
            try
            {
                ...
                if (response.getRequestStatus() == CoreServicesStatus.SUCCESS)
                {
                    ...
                    mView.addObject(Constants.MESSAGE_RESPONSE, this.messageAddArticleSuccess);
                    mView.addObject("command", new Article());
                    mView.setViewName(this.createArticlePage);
                }
                else if (response.getRequestStatus() == CoreServicesStatus.UNAUTHORIZED)
